bool isPlayingDeathSFX = false;  //default no death sound

void noTone(uint8_t pin)
{
  ledcDetachPin(pin);  //detaches the PWM signal from pin
  ledcWrite(0, 0);  //control the duty cycle of the PWM signal, no tone to pin
}
...
void playDeathSound()
{
  ...
  delay(200);
  isPlayingDeathSFX = false;
}

void playDeathSoundTask(void *parameter)
{
  playDeathSound();
  vTaskDelete(NULL); //delete task when done
}

//def diff sound notes
#define NOTE_B0 31
#define NOTE_C1 33
#define NOTE_CS1 35
#define NOTE_D1 37
